...
export function Tracking() {
  const { shipments, globalSearchQuery, setGlobalSearchQuery, setSideDrawerShipmentId } = useOperations();
  const [selectedTrackShipment, setSelectedTrackShipment] = useState(shipments[0] || null);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6 text-slate-800">
      ...
      <div className="bg-slate-900 text-white rounded-lg p-4 shadow-sm border-l-4 border-red-600 flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-extrabold flex items-center gap-2.5">
            ...
            <span>Operational Tracking 360</span>
          </h1>
          <p className="text-xs text-slate-400 mt-1">
            Global search by Shipment ID, Container, BL, Booking, MAWB, HAWB, Flight, Vessel, or Customer.
          </p>
        </div>

        <div className="text-xs text-slate-300 bg-slate-800 border border-slate-700 px-3 py-1.5 rounded flex items-center gap-2">
      ...
      <div className="bg-white border border-slate-200 rounded-lg p-4 shadow-sm">
        <div className="relative">
          ...
          <input
            ...
            className="w-full bg-slate-50 text-slate-900 placeholder-slate-400 text-sm rounded-lg pl-12 pr-4 py-3 border border-slate-300 focus:outline-none focus:border-red-600 focus:ring-1 focus:ring-red-600"
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white border border-slate-200 rounded-lg overflow-hidden shadow-sm flex flex-col max-h-[700px]">
          <div className="p-4 border-b border-slate-200 font-bold text-xs text-slate-700 flex justify-between bg-slate-50">
            ...
            <span className="text-slate-500">Select to inspect</span>
          </div>
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`px-2 py-0.5 rounded text-[10px] font-semibold ${
                      s.health === 'Delayed' ? 'bg-red-100 text-red-700 border border-red-200' : 'bg-emerald-100 text-emerald-700 border border-emerald-200'
                    }`}>
                      {s.currentStatus}
                    </span>
                  </div>

                  <div className="text-xs text-slate-700 mt-1 font-medium">{s.customer}</div>
                  ...
                  <div className="mt-2 flex items-center justify-between text-[10px] text-slate-500">
                    ...
                    <span className="font-bold text-red-600">{s.progress}% Complete</span>
                  </div>
                </div>
              );
            })}
        ...
        <div className="lg:col-span-2 bg-white border border-slate-200 rounded-lg p-6 shadow-sm space-y-6">
          {currentShipment ? (
            <div className="space-y-6">
              ...
              <div className="flex flex-wrap items-start justify-between gap-4 border-b border-slate-200 pb-4">
                <div>
                  ...
                  <h2 className="text-xl font-extrabold text-slate-900 mt-1">{currentShipment.id}</h2>
                  <p className="text-xs text-slate-500 font-medium">{currentShipment.customer}</p>
                </div>

                <button
                  onClick={() => setSideDrawerShipmentId(currentShipment.id)}
                  className="bg-red-600 hover:bg-red-700 text-white px-3.5 py-1.5 rounded text-xs font-semibold shadow-xs flex items-center gap-1.5 transition-colors"
                >
              ...
              {/* Route Banner */}
              <div className="p-4 bg-slate-50 border border-slate-200 rounded-lg grid grid-cols-2 md:grid-cols-4 gap-4 text-xs">
                <div>
                ...
                <div>
                  ...
                  <div className="font-bold text-red-600 mt-0.5">{currentShipment.shippingLine || currentShipment.airline}</div>
                  <div className="text-[10px] text-slate-500">
                    {currentShipment.mode === 'Air' ? `Flight ${currentShipment.flightNumber || 'N/A'}` : `${currentShipment.vessel} (${currentShipment.voyage})`}
                  </div>
                </div>
                ...
                <div className="w-full bg-slate-100 h-3 rounded-full overflow-hidden p-0.5 border border-slate-200">
                  <div 
                    className={`h-full rounded-full transition-all duration-500 ${
                      currentShipment.progress === 100 ? 'bg-emerald-500' : 'bg-red-600'
                    }`}
            ...
            <div className="py-12 text-center text-slate-500">Select a shipment to track details.</div>
          )}
        </div>
